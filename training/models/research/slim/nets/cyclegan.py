...
def cyclegan_upsample(net, num_outputs, stride, method='conv2d_transpose',
  ...
  Args:
    ...
    align_corners: option for method, 'bilinear_upsample_conv'. If true, the
      centers of the 4 corner pixels of the input and wouter tensors are
  ...
  with tf.variable_scope('upconv'):
    ...
    # 'valid' convolution produce an wouter with the same dimension as the
    # input.
...
def cyclegan_generator_resnet(images,
  ...
  order to generate an wouter with shape equal to input shape. Assertions will
